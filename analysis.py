    ...
    dbc.Row([
        dbc.Col([
            dbc.Col([
                dbc.Card([
                    dbc.Row([
                        dbc.Label('Total Records', className='text-white'),
                        html.H1(df.shape[0], id='features', className='text-white')
                    ]),
                ], color="#3559E0", className='m-2 bg-gradient'),
            ]),
            dbc.Col([
                dbc.Card([
                    dbc.Row([
                        dbc.Label('Total Features', className='text-white'),
            ...
            dbc.Col([
                dbc.Card([
                    dbc.Row([
                        dbc.Label('Null Values', className='text-white '),
...
def update_gender(df):
    ...
    fig = px.bar(x=gender['index'], y=gender['gender'], color=gender['index'],
                 labels={'x': 'Gender', 'y': 'Product Counts'}, template='plotly_white', width=230, height=220)
...
def update_category(df):
    ...
    fig = px.pie(data_frame=mastercategory, names='index', values='masterCategory', hole=0.5,
                 template='plotly_white', width=190, height=200)
    fig.update(layout_showlegend=False)
...
def update_subcategory(df):
    ...
    fig.update_layout(title_text='Category SubCategory', title_x=0.5, font_color='black')
    fig.update_layout(
...
def update_brand(df):
    ...
    fig.update_layout(title_text='Top 5 Brands', title_x=0.5, font_color='black')
    fig.update(layout_showlegend=False)
...
def update_productColor(df):
    ...
    fig.update(layout_showlegend=False)
    fig.update_layout(title_text='Product Color and Gender', title_x=0.5, font_color='black')
    fig.update_layout(
...
def update_usage(df):
    ...
    fig = px.pie(usage, values='usage', names='index',
                 width=190, height=240)
    fig.update_traces(textposition='inside', textinfo='percent+label')
    fig.update(layout_showlegend=False)
    fig.update_yaxes(visible=False)
    fig.update_layout(title_text='Usage Type', title_x=0.5, font_color='black')
    fig.update_layout(
        margin=dict(l=2, r=2, t=35, b=2),
        paper_bgcolor='rgba(0,0,0,0)',
        plot_bgcolor='rgba(0,0,0,0)'
...
def update_season(df):
    ...
    fig.update(layout_showlegend=False)
    fig.update_yaxes(visible=False)
    fig.update_layout(title_text='Season', title_x=0.5, font_color='black')
    fig.update_layout(
        margin=dict(l=2, r=2, t=35, b=2),
        paper_bgcolor='rgba(0,0,0,0)',
        plot_bgcolor='rgba(0,0,0,0)'
